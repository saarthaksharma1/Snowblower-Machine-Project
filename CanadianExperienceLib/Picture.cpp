...
void Picture::Save(const wxString& filename)
{
    ...
    if(mMachine1){
        root->AddAttribute(mMachine1->GetName() + L"StartFrame", wxString::Format(wxT("%d"), mMachine1->GetStart()));
        root->AddAttribute(mMachine1->GetName() + L"MachineSelect", wxString::Format(wxT("%d"), mMachine1->GetMachineNumber()));
    }
    if(mMachine2){
        root->AddAttribute(mMachine2->GetName() + L"StartFrame", wxString::Format(wxT("%d"), mMachine2->GetStart()));
        root->AddAttribute(mMachine2->GetName() + L"MachineSelect", wxString::Format(wxT("%d"), mMachine2->GetMachineNumber()));
    }
...
void Picture::Load(const wxString& filename)
{
    ...
    if(mMachine1){
        mMachine1->SetStartFrame(wxAtoi(root->GetAttribute(mMachine1->GetName() + L"StartFrame", L"0")));
        mMachine1->SetMachineNumber(wxAtoi(root->GetAttribute(mMachine1->GetName() + L"MachineSelect", L"1")));
    }

    if(mMachine2){
        mMachine2->SetStartFrame(wxAtoi(root->GetAttribute(mMachine2->GetName() + L"StartFrame", L"0")));
        mMachine2->SetMachineNumber(wxAtoi(root->GetAttribute(mMachine2->GetName() + L"MachineSelect", L"2")));
    }
...
/**
 * Show the dialog box for the machine 1
 * @param window
 */
void Picture::ShowMachine1DialogBox(wxWindow *window)
{

    mMachine1->ShowDialogBox(window);

}

/**
 * Shows the dialog box for the machine 2
 * @param window
 */
void Picture::ShowMachine2DialogBox(wxWindow *window)
{
    mMachine2->ShowDialogBox(window);
}
